...
export default (req: NextApiRequest, res: NextApiResponse) => {
  ...
  const test_data = [{
    ...
    likes: 0,
    like: false,
    content: 'できるだけサーバーレスで開発したい場合の技術選定を考えます。フロント: Next.js SSRで動的',
    name: 'フロントエンジニア',
    mine: true,
    replies: 0
  }, {
    ...
    likes: 1,
    like: false,
    content: 'このサイトは、Next.jsとサーバーレスを掛け合わせる時のテンプレートです。Next.jsでのIS',
    name: 'フロントエンジニア',
    mine: true,
    replies: 1000
  }, {
    ...
    likes: 9999,
    like: true,
    content: 'Firebaseは使いやすいサービスですが、DB周りでRDBとの違いが複数あります。まず、同一レコー',
    name: 'サーバーレス開発',
    mine: false,
    replies: 0
  }, {
  ...
  res.status(200).json({ data: !page ? test_data.slice(0, 3) : (page === 1) ? test_data.slice(3, 6) : [test_data[6]] })
}