...
export default (req: NextApiRequest, res: NextApiResponse<Response>) => {
  ...
  res.status(200).json({ data: (number !== null) ? { follow: test_data[number].follow, follower: test_data[number].follower, mine: test_data[number].mine, following: test_data[number].following } : null })
}