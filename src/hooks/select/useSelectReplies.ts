...
type ResultType = definitions['replies'] & { profiles: definitions['profiles'] } & { replies_likes: definitions['replies_likes'][] | undefined }

const FetchData = async (pageParam: string | undefined, comment_id: definitions['comments']['id']) => {
...
const useSelectReplies = (id: definitions['comments']['id']) => {
  ...
  const { data, isFetching, hasNextPage, fetchNextPage } = useInfiniteQuery(['replies', id],
