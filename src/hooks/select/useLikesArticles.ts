...
const useLikesArticles = (path: string) => {
  ...
  const { data, isFetching, hasNextPage, fetchNextPage } = useInfiniteQuery(['likes_articles', path],
    ({ pageParam }) => FetchData(pageParam, path), {
      ...
      getNextPageParam: (lastPage) => (lastPage && (lastPage.length === 10)) ? lastPage[lastPage.length - 1].likes_created : false
    },
  )
