...
type ResultType = definitions['comments'] & { profiles: definitions['profiles'] } & { comments_likes: definitions['comments_likes'][] | undefined }
...
const useSelectComments = (path: string) => {
  ...
  const { data, refetch, isFetching, hasNextPage, fetchNextPage } = useInfiniteQuery(['comments', path],
