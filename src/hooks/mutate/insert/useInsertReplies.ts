...
type CommentsExistingType = InfiniteData<[definitions['comments'] & { profiles: definitions['profiles'] } & { comments_likes: definitions['comments_likes'][] | undefined }]> | undefined
type RepliesExistingType = InfiniteData<[definitions['replies'] & { profiles: definitions['profiles'] } & { replies_likes: definitions['replies_likes'][] | undefined }]> | undefined
...
const useInsertReplies = (path: string, comment_id: number) => {
  ...
  const { mutate, isLoading } = useMutation((comment: string) => Mutate({ comment_id, comment }), {
    onSuccess: data => {
      ...
      const replies_existing: RepliesExistingType = queryClient.getQueryData(['replies', comment_id])

      if(comments_existing) {
        // 返信した対象のコメントをfindIndexで探す
        const index = comments_existing.pages.map(page => page.findIndex(item => item.id === comment_id))
        const page = index.findIndex(item => item !== -1)

        // キャッシュから対象のコメントのリプライ数を+1する
        comments_existing.pages[page][index[page]].reply_count = comments_existing.pages[page][index[page]].reply_count + 1
